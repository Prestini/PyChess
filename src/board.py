...
class Board:
    ...
    def __init__(self) -> None:
        ...
        self.last_move = None
        self._create_board()
        self._add_pieces(STARTING_POSITION['fen'])
        self.king_pos = {
            'b': STARTING_POSITION['k'],
            'w': STARTING_POSITION['K']
        }
    ...
    def move(self, piece, move, testing=False):
        ...
        if isinstance(piece, King):
            ...
            self.king_pos[piece.color] = (final.row, final.col)
        ...
        for row in range(ROWS):
            for col in range(COLS):
                if temp_board.squares[row][col].has_enemy_piece(piece.color):
                    ...
                    temp_board.calc_moves(p, row, col, do_weird_stuff=False)
                    for op_move in p.moves:
                        if isinstance(op_move.final.piece, King):
                            return True
        
        return False
    
    def get_king_pos(self, color):
        return self.king_pos[color]

    def calc_moves(self, piece, row, col, do_weird_stuff=True):
        '''
            Calculate all the possible (valid) moves of an specific piece on a specific position
        '''

        # TODO Implement this idea:
        # First, check if move leaves us in check next move, if so, illegal
                ...
                if isinstance(left_rook, Rook):
                    if not left_rook.moved:
                        for c in range(1, 4):
                            ...
                            # castling is not possible because check would happen
                            this_move = Move(Square(row, c+1), Square(row, c))
                            if self.in_check(piece,this_move):
                                break
